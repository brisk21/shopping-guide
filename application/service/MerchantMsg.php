...
class MerchantMsg
{
    ...
    public static function add($uid, $data)
    {
        ...
        return AppCommon::data_add('plugin_merchant_msg', [
            'uid' => $uid,
            'add_time' => time(),
    ...
    {
        return AppCommon::data_update('plugin_merchant_msg', ['id' => intval($id)], $data);
    }

    /**
     * 删除消息
     * @param int|array $id 某记录ID
    ...
    public static function del($id = 0, $uid = '', $isAll = false)
    {
        ...
        if ($isAll) {
            return AppCommon::data_del('plugin_merchant_msg', ['uid' => $uid]);
        }
        if (!is_array($id)) {
            $id = [$id];
        }
        return AppCommon::data_del('plugin_merchant_msg', ['id' => ['in', $id]]);
    }
}
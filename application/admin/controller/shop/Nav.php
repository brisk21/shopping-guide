...
class Nav extends Admin
{
    public function index()
    {
        ...
        $data = AppCommon::data_list('navs', $where, $page . ',' . $pageSize, '*', $orderBy);

        $total = AppCommon::data_count('navs', $where);
    ...
    public function form()
    {
        ...
        if (!empty($id)) {
            $data = AppCommon::data_get('navs', ['id' => $id]);
            if (!empty($data)) {
    ...
    public function action_status()
    {
        ...
        $data = AppCommon::data_get('navs', ['id' => intval($this->param['id'])]);
        if (empty($data['id'])) {
            data_return('记录不存在', -1);
        }
        $res = AppCommon::data_update('navs', ['id' => $data['id']], [
            'status' => $data['status'] == 0 ? -1 : 0
        ]);
        if ($res) {
            parent::add_admin_log(['title' => '修改导航状态', 'content' => ($data['status'] == 0 ? '下线' : '展示')]);
        }
    ...
    public function nav_action()
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($this->param['ac']) && $this->param['ac'] == 'del') {
                AppCommon::data_del('navs', ['id' => $data['id']]);
                parent::add_admin_log(['title' => '删除导航', 'content' => $data]);
                data_return('删除成功');
            }
        }
        ...
        if (!empty($data)) {
            AppCommon::data_update('navs', ['id' => $data['id']], [
        ...
        parent::add_admin_log(['title' => '操作导航', 'content' => $this->param]);
        data_return('操作成功');
    }
}
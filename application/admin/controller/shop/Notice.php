...
class Notice extends Admin
{
    public function index()
    {
        ...
        $data = AppCommon::data_list('article', $where, $page . ',' . $pageSize, 'title,id,status,count_view,add_time,up_time', $orderBy);
        ...
        $total = AppCommon::data_count('article', $where);
    ...
    public function action_status()
    {
        ...
        $data = AppCommon::data_get('article', ['id' => intval($this->param['id'])]);
        if (empty($data['id'])) {
            data_return('记录不存在', -1);
        }
        $res = AppCommon::data_update('article', ['id' => $data['id']], [
            'up_time' => time(),
            'status' => $data['status'] == 1 ? 0 : 1
        ]);
        if ($res) {
            parent::add_admin_log(['title' => '修改文章状态', 'content' => ($data['status'] == 1 ? '下线' : '展示')]);
        }
    ...
    public function form()
    {
        ...
        if (!empty($id)) {
            $data = AppCommon::data_get('article', ['id' => $id]);
            if (!empty($data)) {
               $this->assign('data',$data);
            }
    ...
    public function notice_action()
    {
        ...
        if (!empty($data)) {
            AppCommon::data_update('article', ['id' => $data['id']], [

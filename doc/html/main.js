    ...
    apiGroups.forEach(function(groupEntry) {
        ...
        api.forEach(function(entry) {
            ...
            if(groupEntry === entry.group) {
                ...
                //移除右侧文章接口的标题自定义html内容
                fields.article.title = escape2Html(fields.article.title).replace(/<newapi[^>]*>(.|\n)*<\/newapi>/gim, '');
                fields.article.title = escape2Html(fields.article.title).replace(/<unuse[^>]*>(.|\n)*<\/unuse>/gim, '');
                fields.article.title = escape2Html(fields.article.title).replace(/<docoding[^>]*>(.|\n)*<\/docoding>/gim, '');
